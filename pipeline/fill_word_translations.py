...
PAIR_REGISTRY: dict[tuple[str, str], dict[str, Any]] = {
    ...
    # ── Russian → Spanish (kaikki_1) ──────────────────────────────────────────
    ("ru", "es"): {
        "backend": "kaikki",
        "db_candidates": [
            "backend/dictionaries/ru_es/ru_es.db",
    ...
    # ── English → German (kaikki_1) ───────────────────────────────────────────
    # Uses the eval pipeline's Lookup directly: overrides (~280 entries) +
    # spaCy lemmatization + contraction expansion + German POS post-filter +
    # kaikki DB (160,371 rows, 100% coverage on song 240).
    ("en", "de"): {
        "backend": "en_de",
        "db_candidates": [
            "backend/dictionaries/en_de/en_de.db",
            "eval/pipelines/en_de/kaikki_1/data/en_de.db",
        ],
    },
    # ── English → Portuguese (kaikki_1) ────────────────────────────────────────
    # Uses the eval pipeline's Lookup directly: overrides (~290 entries) +
    # spaCy lemmatization + contraction expansion +
    # kaikki DB (100,913 rows, 100% coverage on song 240).
    ("en", "pt"): {
        "backend": "en_pt",
        "db_candidates": [
            "backend/dictionaries/en_pt/en_pt.db",
            "eval/pipelines/en_pt/kaikki_1/data/en_pt.db",
        ],
    },
...
# ── Backend: en_de (spaCy + overrides + German POS filter + kaikki DB) ────────
...
def fill_en_de(
    ...
    """
    English → German fill using the eval pipeline's Lookup directly.

    Brings ~280-entry overrides for function words, spaCy lemmatization,
    contraction expansion, and a German POS post-filter (noun capitalisation,
    verb infinitive preference).
    """
    db_path = _resolve_kaikki_db(config, db_path_override)
    if db_path is None:
        candidates = "\n".join(f"  {REPO_ROOT / r}" for r in config.get("db_candidates", []))
        sys.exit(
            f"en_de DB not found.\nExpected at:\n{candidates}\n"
            "Build it first: python -m eval.pipelines.en_de.kaikki_1.build_db"
        )

    from eval.pipelines.en_de.kaikki_1.lookup import Lookup  # type: ignore[import]

    _log(f"Backend: en_de | DB: {db_path}")
    lookup = Lookup(src_lang, tgt_lang, db_path=db_path)
    _log("  Lookup ready (spaCy + overrides + German POS filter + kaikki DB).")
...
# ── Backend: en_pt (spaCy + overrides + kaikki DB) ────────────────────────
...
def fill_en_pt(
    ...
    """
    English → Portuguese fill using the eval pipeline's Lookup directly.

    Brings ~290-entry overrides for function words, spaCy lemmatization,
    contraction expansion, and kaikki DB (100,913 rows).
    """
    db_path = _resolve_kaikki_db(config, db_path_override)
    if db_path is None:
        candidates = "\n".join(f"  {REPO_ROOT / r}" for r in config.get("db_candidates", []))
        sys.exit(
            f"en_pt DB not found.\nExpected at:\n{candidates}\n"
            "Build it first: python -m eval.pipelines.en_pt.kaikki_1.build_db"
        )

    from eval.pipelines.en_pt.kaikki_1.lookup import Lookup  # type: ignore[import]

    _log(f"Backend: en_pt | DB: {db_path}")
    lookup = Lookup(src_lang, tgt_lang, db_path=db_path)
    _log("  Lookup ready (spaCy + overrides + kaikki DB).")
