"""
en→de lookup with spaCy-powered in-context lemmatization and POS disambiguation.

Mirrors eval/pipelines/en_es/kaikki_1/lookup.py but targets German (de_word).
Adds a German-specific POS post-filter:
  - nouns → prefer entries starting with an uppercase letter
  - verbs → prefer entries ending in -en / -eln / -ern (infinitive form)
  - adj/adv → prefer lowercase entries
...
Run eval:
    python -m eval.run --pipeline en_de/kaikki_1 --src en --tgt de --song-id 240
"""
...
from .overrides import EN_DE_OVERRIDES

_HERE = Path(__file__).parent
_REPO_ROOT = _HERE.parents[3]   # eval/pipelines/en_de/kaikki_1 → repo root

DEFAULT_DB = _HERE / "data" / "en_de.db"
_PROD_DB = _REPO_ROOT / "backend" / "dictionaries" / "en_de" / "en_de.db"
...
# "to" added to skip the infinitive marker in "wanna"/"gonna"/"gotta" expansions.
...
def _find_db() -> Path:
    ...
    raise FileNotFoundError(
        "en_de.db not found.\n"
        "Build it first:  python -m eval.pipelines.en_de.kaikki_1.build_db"
    )
...
def _de_pos_filter(words: list[str], pos: str | None) -> list[str]:
    """Reorder German results so the most grammatically plausible form appears
    first.  German nouns are always capitalised; infinitives end in -en/-eln/-ern.
    If the filter would produce an empty list, return the original list unmodified.
    """
    if not pos or not words:
        return words
    if pos == "noun":
        preferred = [w for w in words if w and w[0].isupper()]
    elif pos == "verb":
        preferred = [
            w for w in words
            if w and (
                (w.lower().endswith(("en", "eln", "ern")) and len(w) >= 3)
                or w.lower().startswith("sich ")
            )
        ]
    elif pos in ("adj", "adv"):
        preferred = [w for w in words if w and w[0].islower()]
    else:
        return words
...
class Lookup:
    """
    en→de lookup combining spaCy in-context lemmatization with the kaikki DB.
    """
    ...
    def __init__(self, src: str, tgt: str, db_path: Path | None = None) -> None:
        ...
        try:
            ...
            print(f"[en_de lookup] spaCy unavailable ({exc}); falling back to DB-only lookup.")
            self._nlp = None
    ...
    def _db_lookup(self, lemma: str, pos: str | None) -> list[str]:
        """Return up to _MAX_RESULTS German translations, POS-preferred.

        Applies a German-specific post-filter (_de_pos_filter) to surface
        more linguistically accurate forms (capitalised nouns, -en infinitives)
        while still falling back to unfiltered results when the filter empties.
        """
        seen: set[str] = set()
        ...
        if pos:
            ...
            result = _dedup(rows)
            if result:
                filtered = _de_pos_filter(result, pos)
        ...
        result = _dedup(rows)
        return _de_pos_filter(result, pos)[:_MAX_RESULTS]
    ...
    def lookup_line(self, line_text: str, words: list) -> None:
        ...
        for word in words:
            ...
            if expanded:
                ...
                for part in expanded:
                    if part in _CONTRACTION_SILENT:
                        continue
                    ov_part = EN_DE_OVERRIDES.get(part)
            ...
            if pool_entry:
                ...
                ov = EN_DE_OVERRIDES.get(spacy_lemma)
                if ov is not None:
                    word.tr_definitions = ov
                    continue
    ...
    def lookup(self, lemma: str, grammar: str = "") -> list[str]:
        ...
        ov = EN_DE_OVERRIDES.get(clean.lower())
        if ov is not None:
            return ov
