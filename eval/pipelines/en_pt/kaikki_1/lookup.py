"""
en→pt lookup with spaCy-powered in-context lemmatization and POS disambiguation.

Mirrors eval/pipelines/en_es/kaikki_1/lookup.py but targets Portuguese (pt_word)
instead of Spanish.  Portuguese is a Romance language; no special POS post-filter
is needed (unlike German noun capitalisation), but the override table is tuned
for neutral Brazilian/European Portuguese.

Requires:
    python -m spacy download en_core_web_sm
    python -m eval.pipelines.en_pt.kaikki_1.build_db

Run eval:
    python -m eval.run --pipeline en_pt/kaikki_1 --src en --tgt pt --song-id 240
"""
...
from .overrides import EN_PT_OVERRIDES

_HERE = Path(__file__).parent
_REPO_ROOT = _HERE.parents[3]   # eval/pipelines/en_pt/kaikki_1 → repo root

DEFAULT_DB = _HERE / "data" / "en_pt.db"
_PROD_DB = _REPO_ROOT / "backend" / "dictionaries" / "en_pt" / "en_pt.db"
...
# ---------------------------------------------------------------------------
# Auxiliary / function words skipped during contraction expansion
# ---------------------------------------------------------------------------
...
class Lookup:
    """
    en→pt lookup combining spaCy in-context lemmatization with the kaikki DB.
    """
    ...
    def __init__(self, src: str, tgt: str, db_path: Path | None = None) -> None:
        ...
        try:
            ...
            print(f"[en_pt lookup] spaCy unavailable ({exc}); falling back to DB-only lookup.")
            self._nlp = None
    ...
    def _db_lookup(self, lemma: str, pos: str | None) -> list[str]:
        """Return up to _MAX_RESULTS Portuguese translations, POS-preferred."""
        seen: set[str] = set()
        ...
        if pos:
            rows = self._conn.execute(
                "SELECT DISTINCT pt_word FROM definitions WHERE lemma = ? AND pos = ?",
                (lemma, pos),
            ).fetchall()
            result = _dedup(rows)
            if result:
        ...
        rows = self._conn.execute(
            "SELECT DISTINCT pt_word FROM definitions WHERE lemma = ?",
            (lemma,),
        ).fetchall()
    ...
    def lookup_line(self, line_text: str, words: list) -> None:
        ...
        for word in words:
            ...
            if expanded:
                ...
                for part in expanded:
                    if part in _CONTRACTION_SILENT:
                        continue
                    ov_part = EN_PT_OVERRIDES.get(part)
            ...
            if pool_entry:
                ...
                ov = EN_PT_OVERRIDES.get(spacy_lemma)
                if ov is not None:
                    word.tr_definitions = ov
                    continue
    ...
    def lookup(self, lemma: str, grammar: str = "") -> list[str]:
        ...
        ov = EN_PT_OVERRIDES.get(clean.lower())
        if ov is not None:
            return ov
