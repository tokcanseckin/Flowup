"""
Build a local en→pt SQLite DB from the enwiktionary raw wiktextract dump.

Source dump (shared with en_es, en_de, en_ru):
...
def _extract_pt_rows(entry: dict, lemma: str, pos: str) -> list[tuple]:
    """Return (lemma, pos, pt_word, pt_sense) tuples for all Portuguese translations.

    Translations tagged as slang, dialectal, vulgar, archaic, etc. are excluded
    so the DB contains only standard (neutral Portuguese / Brazilian) entries.
    """
    rows: list[tuple] = []

    def _is_pt(t: dict) -> bool:
        lang_code = t.get("lang_code", "")
        lang_name = t.get("lang", "").lower()
        if not (
            lang_code == "pt"
            or lang_name in ("portuguese", "português")
        ):
            return False
        if not t.get("word"):
            return False
    ...
    def _clean_pt_word(word: str) -> str:
        # Strip parenthetical gender markers like "(m)", "(f)", "(mf)"
        import re
        word = re.sub(r"\s*\([mfn]\w*\)\s*", " ", word).strip()
        return word.strip()

    # Top-level translations
    for t in entry.get("translations", []):
        if _is_pt(t):
            pt_word = _clean_pt_word(t["word"])
            sense_text = t.get("sense") or ""
            if pt_word:
                rows.append((lemma, pos, pt_word, sense_text))
...
def build_db(source: Path, db_path: Path) -> None:
    ...
    with opener(source, "rt", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            pos = entry.get("pos", "")
            rows = _extract_pt_rows(entry, lemma, pos)

            if rows:
                conn.executemany(
                    "INSERT INTO definitions (lemma, pos, pt_word, pt_sense) VALUES (?,?,?,?)",
                    rows,
                )
                inserted += len(rows)
            ...
            if (i + 1) % 100_000 == 0:
                conn.commit()
                print(f"  {i + 1:,} lines … {inserted:,} PT rows so far "
                      f"(skipped {skipped_lang:,} non-en)")

    conn.commit()
    conn.close()
    print(f"\nDone. {inserted:,} en→pt rows written to {db_path}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build en→pt SQLite DB from the enwiktionary raw dump."
    )
    parser.add_argument(
