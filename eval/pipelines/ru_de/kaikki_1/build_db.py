"""
Build a local ru→de SQLite DB from the ruwiktionary raw wiktextract dump.

Source dump (shared with ru_tr/kaikki_1):
...
Build two-hop hop DBs (ru_en + en_de):
    python -m eval.pipelines.ru_de.kaikki_1.build_db --build-hops
...
Options:
    ...
    --build-hops    Also build ru_en.db and en_de.db for two-hop fallback
"""
...
# Hop DB paths (for two-hop ru→en→de fallback)
RU_EN_DB = _HERE / "data" / "ru_en.db"
EN_DE_DB = _HERE / "data" / "en_de.db"

EN_SOURCE = _EVAL_ROOT / "sources" / "enwiktionary" / "raw-wiktextract-data.jsonl.gz"

# Tags in enwiktionary that indicate non-standard / dialectal German.
# Translations carrying any of these tags are skipped when building en_de.db.
_DE_DIALECT_TAGS: frozenset[str] = frozenset({
    "Alemannic-German", "Bavarian", "Austrian-German", "Swiss-German",
    "Low-German", "Ripuarian", "Colognian", "Palatine-German",
    "Silesian-German", "Westphalian", "Northern-German", "Southern-German",
    "Hunsrik", "Luxembourgish",
})
...
def _extract_de_rows(entry: dict, lemma: str, pos: str) -> list[tuple]:
    ...
    def _is_de(t: dict) -> bool:
        return (
            t.get("lang_code") == "de"
            or t.get("lang", "").lower() in ("german", "немецкий")
        ) and bool(t.get("word"))
    ...
    for t in entry.get("translations", []):
        ...
        if _is_de(t) and not _is_technical(sense_text):
...
def build_db(source: Path, db_path: Path) -> None:
    ...
    with opener(source, "rt", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            if (i + 1) % 100_000 == 0:
                conn.commit()
                print(f"  {i + 1:,} lines … {inserted:,} DE rows so far "
                      f"(skipped {skipped_lang:,} non-ru)")

    conn.commit()
    conn.close()
    print(f"\nDone. {inserted:,} ru→de rows written to {db_path}")


def _extract_hop_rows(entry: dict, lemma: str, trans_lang_code: str, skip_dialect_tags: bool = False) -> list[tuple]:
    ...
    def _has_dialect_tag(t: dict) -> bool:
        return skip_dialect_tags and bool(
            _DE_DIALECT_TAGS.intersection(t.get("tags") or [])
        )

    for t in entry.get("translations", []):
        if _is_target(t) and not _has_dialect_tag(t):
...
def build_hop_db(
    ...
    skip_dialect_tags: bool = False,
) -> None:
    """Build a single-hop lemma→tgt_word DB (e.g. ru→en or en→de)."""
    ...
    with opener(source, "rt", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            rows = _extract_hop_rows(entry, lemma, trans_lang_code, skip_dialect_tags=skip_dialect_tags)
            if rows:
                conn.executemany(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build ru→de SQLite DB from the ruwiktionary raw dump."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Also build ru_en.db (from ruwiktionary) and en_de.db (from enwiktionary) "
            "for the two-hop ru→en→de fallback."
        ),
    )
    ...
    if args.build_hops:
        ...
        if not EN_SOURCE.exists():
            print(f"SKIP en→de: {EN_SOURCE} not found")
        else:
            build_hop_db(EN_SOURCE, EN_DE_DB, "en", "de", skip_dialect_tags=True)
